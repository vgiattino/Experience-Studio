...
**[revised by implementation] The immutability qualifier is load-bearing, and testing the artifact's own flag is not enough.** A cache keyed on `(id, artifactVersion)` is sound only for an artifact that cannot change under that key. A draft mutates while keeping both, so `compilePage` caches only when `version.immutable !== false` — necessary, and *insufficient*: the builder opens a **published** definition and mutates the working copy, which carries `immutable: true` and defeats the check. The rule that actually holds is about provenance rather than a flag: **a definition handed over in memory has no version identity at all**, because its holder is free to change it while keeping both fields. `PageLoaderService.loadDefinition` therefore bypasses the cache unconditionally, and `PageRendererComponent`'s attach guard keys on the compiled page's object identity rather than its `cacheKey`. Compilation is sub-millisecond, so the cost is nil; the symptoms of getting it wrong were a canvas frozen at the version first loaded and a newly added data source that was never queried.
